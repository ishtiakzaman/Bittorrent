...
typedef struct {
  ...
  char peerArray[MAX_CONNECTIONS][FILE_NAME_MAX];
  char torrent_file[FILE_NAME_MAX];// *.torrent file
  std::map<int, peer_t*> peers; // array of peer_t pointers
  unsigned char id[20]; //this bt_clients id
  ...
  struct sockaddr_in ownaddr; // Own address
