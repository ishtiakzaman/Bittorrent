...
void __parse_peer(peer_t *peer, char *peer_st){
  ...
  //need to copy because strtok mangles things
...
void parse_args(bt_args_t *bt_args, int argc,  char *argv[]){
  ...
  memset(bt_args->logFile,0x00,FILE_NAME_MAX);
  ...
  //default log file
  strncpy(bt_args->logFile,"bt-client.log",FILE_NAME_MAX);

  for(i=0;i<MAX_CONNECTIONS;i++){
    bt_args->peers[i] = NULL; //initially NULL
  }

  strcpy((char *)bt_args->id, "");
  bt_args->n_peer = 0;
  bt_args->seed = 1;
  ...
  while ((ch = getopt(argc, argv, "hp:s:l:vI:")) != -1) {
    ...
    case 'p': //peer
		...
		if(n_peers+1 > MAX_CONNECTIONS){
			fprintf(stderr,"ERROR: Can only support %d initial peers",MAX_CONNECTIONS);
			usage(stderr);
			exit(1);
		}
		strncpy(bt_args->peerArray[bt_args->n_peer], optarg, FILE_NAME_MAX);
		// This is a leecher
		bt_args->seed = 0;
    ...
    case 'l'://log file
      memset(bt_args->logFile,0x00,FILE_NAME_MAX);
      strncpy(bt_args->logFile, optarg, FILE_NAME_MAX);
      break;
    case 'I':
	  strcpy((char *)bt_args->id, optarg);
      break;
    default:
